...
def resolve_job_dir() -> Path:
    ...
    for base in bases:
        candidates = [base / "jobs" / "policy_ingestion", base]
        candidates.extend(base.glob("*/jobs/policy_ingestion"))
        for candidate in candidates:
            if (candidate / "run_job.py").is_file():
                return candidate
    raise RuntimeError("Unable to locate jobs/policy_ingestion from the CAI project")


HERE = resolve_job_dir()
VENV = HERE / ".venv-policy-ingestion"
...
def main() -> None:
    initialize = os.getenv("POLICY_JOB_INIT_SCHEMA", "false").lower() == "true"
    script = "init_schema.py" if initialize else "run_job.py"
    command = [str(ensure_venv()), str(HERE / script)]
    if not initialize and os.getenv("POLICY_JOB_DRY_RUN", "true").lower() == "true":
