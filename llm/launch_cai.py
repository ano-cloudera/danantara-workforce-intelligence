...
def main():
    ...
    env = os.environ.copy()
    env.setdefault("LLM_MODEL_ID", "Qwen/Qwen3-14B-AWQ")
    env.setdefault("LLM_MODEL_DIR", str(package_dir / "models" / "qwen3-14b-awq"))
    env.setdefault("EMBED_MODEL_ID", "BAAI/bge-m3")
    env.setdefault("EMBED_MODEL_DIR", str(package_dir / "models" / "bge-m3"))
    env.setdefault("LLM_TENSOR_PARALLEL_SIZE", "1")
    env.setdefault("LLM_QUANTIZATION", "awq_marlin")
    env.setdefault("LLM_GPU_MEMORY_UTILIZATION", "0.90")
    env.setdefault("EMBED_NUM_REPLICAS", "1")
    env.setdefault("EMBED_GPU_PER_REPLICA", "1.0")
    env.setdefault("MAX_MODEL_LEN", "8192")
    env.setdefault("MAX_NUM_SEQS", "16")
    env.setdefault("DTYPE", "auto")
