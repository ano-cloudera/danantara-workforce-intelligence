...
async function openCandidateDetail(candidateId) {
  ...
  try {
    ...
    $("#candidate-detail").innerHTML = `<header class="candidate-detail-header"><span class="candidate-avatar">${escapeHtml(initials)}</span><div><span class="eyebrow">SAFE POC PROFILE</span><h2>${escapeHtml(candidate.name)}</h2><p>${escapeHtml(candidate.current_title || "Current title unavailable")} · ${escapeHtml(candidate.company || "Entity unavailable")}</p></div></header>
      ...
      <section class="candidate-detail-section"><h3>Skills and sample proficiency</h3><div class="proficiency-list">${proficiency.map(([skill, score]) => `<div><span>${escapeHtml(skill)}</span><progress max="5" value="${escapeHtml(score)}"></progress><strong>${escapeHtml(score)}/5</strong></div>`).join("") || "<p class='muted'>No skill data.</p>"}</div></section>
      <section class="candidate-detail-section"><h3>Experience</h3>${(candidate.experiences || []).map(item => `<article class="experience-item"><strong>${escapeHtml(item.role)}</strong><span>${escapeHtml(item.employer)} · ${escapeHtml(item.start_year)}–${escapeHtml(item.end_year || "Present")}</span><p>${escapeHtml(item.summary)}</p></article>`).join("") || "<p class='muted'>No experience history.</p>"}</section>
