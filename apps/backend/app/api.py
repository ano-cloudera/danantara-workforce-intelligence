...
from app.models import (
    CandidateForm, FeedbackRequest, PolicyQueryRequest, PolicyQueryResponse, PolicySource,
    PublicConfig, TalentMatchRequest, TalentMatchResponse
)
...
def build_router(services: dict, settings: Settings) -> APIRouter:
    ...
    def public_config():
        return PublicConfig(
            ...
            services={'qdrant_healthy': services['qdrant'].healthy(), 'observability_configured': bool(settings.observability_url)},
        )
    ...
    def talent_match(payload: TalentMatchRequest, x_user_id: str | None = Header(default=None)):
        ...
        input_guard = services['guardrails'].validate_input((payload.position_title or '') + ' ' + ' '.join(payload.skills_keywords))
        if not input_guard.allowed:
            raise HTTPException(400, detail={'message': 'Input blocked by guardrail', 'guardrail': input_guard.model_dump()})
        flow = TalentMatchingFlow(payload, services['data'], services['gemini'], services['guardrails'], services['obs'])
        flow.kickoff()
        matches = flow.state.final
        output_guard = services['guardrails'].validate_talent_output(len(matches))
        return TalentMatchResponse(request_id=request_id, session_id=sid, position=flow.state.position, matches=matches, guardrail=output_guard, human_review_required=True)
    ...
    def policy_query(payload: PolicyQueryRequest, x_user_id: str | None = Header(default=None)):
        ...
        if not input_guard.allowed:
            raise HTTPException(400, detail={'message': 'Input blocked by guardrail', 'guardrail': input_guard.model_dump()})
        flow = PolicyRAGFlow(payload, services['qdrant'], services['policy_fallback'], services['gemini'], services['obs'])
        flow.kickoff()
        sources = [PolicySource(**s) for s in flow.state.sources]
        output_guard = services['guardrails'].validate_policy_output(flow.state.answer, len(sources))
        citations = [f"[{i}] {s.title}" for i,s in enumerate(sources, start=1)]
        return PolicyQueryResponse(request_id=request_id, session_id=sid, answer=flow.state.answer, sources=sources, citations=citations, guardrail=output_guard, human_review_required=True)
    ...
    def candidate_form(payload: CandidateForm):
        ...
        return {'status': 'accepted', 'submission_id': item_id, 'routing': 'nifi' if settings.ingest_mode == 'nifi' else 'backend_poc_fallback'}

    @router.post('/sources/upload')
    async def source_upload(file: UploadFile = File(...), entity: str | None = Form(default=None), doc_type: str | None = Form(default=None)):
        content = await file.read()
        if len(content) > 50 * 1024 * 1024:
            raise HTTPException(413, 'File exceeds 50 MB PoC limit')
        return services['ingestion'].save_and_process(file.filename or 'upload.pdf', content, entity, doc_type)

    @router.post('/feedback')
    def feedback(payload: FeedbackRequest):
        services['store'].add_feedback(payload.model_dump())
        services['obs'].emit('feedback', 'user-feedback', {'rating': payload.rating, 'has_comment': bool(payload.comment)})
